...
[INFO] 14.06.2026 | RESPONSIVE_TEXT_STAGE_START | Начат этап проверки заголовков
экранов и текстов кнопок на устройствах с разной шириной и масштабом шрифта | IN_PROGRESS

[INFO] 14.06.2026 | RESPONSIVE_SCREEN_TITLES | Добавлен общий адаптивный стиль
заголовков экранов с диапазоном 20–30sp, автоматическим подбором размера и поддержкой
до двух строк; стиль применён к auth, client, trainer и admin экранам | OK

[INFO] 14.06.2026 | RESPONSIVE_BUTTON_TEXT | Основные и контурные Material-кнопки
получили автоматический подбор текста 10–15sp, центрирование, поддержку двух строк и
минимальную высоту касания 48dp | OK

[INFO] 14.06.2026 | COMPACT_BUTTON_TOUCH_TARGETS | Кнопки высотой 40–44dp в списках,
карточках и административных разделах увеличены до 48dp без изменения логики действий | OK

[INFO] 14.06.2026 | QUICK_ACTION_LABELS | Подписи быстрых действий получили отдельный
адаптивный стиль; длинное `РАСПИСАНИЕ` заменено на компактное `ЗАНЯТИЯ`, чтобы исключить
разрыв слова на узких экранах | OK

[INFO] 14.06.2026 | RESPONSIVE_NARROW_DEVICE_CHECK | На профиле 720x1280,
density 320 и font scale 1.3 проверены главная, расписание, записи и профиль:
заголовки, карточки, нижняя навигация и кнопки отображаются без обрезания | OK

[INFO] 14.06.2026 | RESPONSIVE_STANDARD_DEVICE_CHECK | После проверки Pixel 10
возвращён к 1280x2856, density 480 и font scale 1.0; штатный интерфейс визуально
проверен, исходная иерархия размеров сохранена | OK

[INFO] 14.06.2026 | RESPONSIVE_TEXT_BUILD_CHECK | Выполнены lintDebug,
testDebugUnitTest и assembleDebug; сборка завершена успешно | OK

[INFO] 14.06.2026 | RESPONSIVE_TEXT_STAGE_COMPLETE | Адаптация заголовков экранов,
подписей быстрых действий и текстов кнопок завершена и проверена | COMPLETE

[INFO] 14.06.2026 | DEMO_SEED_STAGE_START | Начат этап подготовки тестовых данных
для визуальной и функциональной проверки приложения | IN_PROGRESS

[INFO] 14.06.2026 | DEMO_SEED_DATASET | Добавлен `database/seed_demo.sql`,
создающий две группы, шесть упражнений, три WOD, пять занятий, записи,
посещаемость и результаты относительно текущей даты | OK

[INFO] 14.06.2026 | DEMO_SEED_AUTH_SAFETY | Seed использует существующие профили
атлета и тренера, не создаёт записи `auth.users`, не хранит пароли и позволяет
явно выбрать аккаунты по email | OK

[INFO] 14.06.2026 | DEMO_SEED_REPEATABILITY | Скрипт использует фиксированные
demo-ID и при повторном запуске заменяет только собственный набор данных;
изменения выполняются в одной транзакции | OK

[INFO] 14.06.2026 | DEMO_SEED_GUIDE | `database/README.md` дополнен требованиями,
порядком запуска в Supabase SQL Editor и описанием создаваемых данных | OK

[INFO] 14.06.2026 | DEMO_SEED_VALIDATION | SQL-валидатор расширен проверкой
`seed_demo.sql` и PL/pgSQL DO-блоков; проверено 252 PostgreSQL statements и
11 PL/pgSQL blocks/functions | OK

[INFO] 14.06.2026 | DEMO_SEED_STAGE_COMPLETE | Идемпотентный набор тестовых данных
подготовлен и синтаксически проверен; применение в Supabase ожидает пользователя | COMPLETE

[INFO] 16.06.2026 | ROLE_PROFILE_STAGE_START | Начат этап добавления профиля
и выхода из аккаунта для ролей тренера и администратора | IN_PROGRESS

[INFO] 16.06.2026 | TRAINER_PROFILE_NAV | В нижнее меню и navigation graph
тренера добавлена вкладка `Профиль`, переиспользующая общий ProfileFragment с
существующей logout-логикой | OK

[INFO] 16.06.2026 | ADMIN_PROFILE_NAV | В нижнее меню и navigation graph
администратора добавлена вкладка `Профиль`, переиспользующая общий ProfileFragment
с существующей logout-логикой | OK

[INFO] 16.06.2026 | ROLE_PROFILE_BUILD_CHECK | Выполнены lintDebug,
testDebugUnitTest и assembleDebug; сборка завершена успешно | OK
...
[INFO] 16.06.2026 | WOD_TASK_UI_STAGE_START | Начат этап переделки UI конструктора WOD
под задания, типы нагрузки и RX/Optional-варианты | IN_PROGRESS

[INFO] 16.06.2026 | WOD_TASK_EDITOR_UI | `fragment_wod_editor.xml` и `WodEditorFragment`
переведены со старой формы упражнение/раунды/вес на форму задания: название, RX упражнение,
тип нагрузки, RX нагрузка, Optional упражнение, Optional нагрузка и комментарий | OK

[INFO] 16.06.2026 | WOD_TASK_EDITOR_SAVE | `WodEditorViewModel` сохраняет WOD через
`create_wod_with_tasks(...)`; справочник `load_types` загружается вместе с группами и упражнениями | OK

[INFO] 16.06.2026 | ATHLETE_WOD_TASK_DISPLAY | Главный экран атлета отображает новый список
`wod_tasks` с RX/Optional-вариантами и сохраняет fallback для старых `wod_exercises` | OK

[INFO] 16.06.2026 | WOD_TASK_UI_BUILD_CHECK | Выполнены `lintDebug`, `testDebugUnitTest`
и `assembleDebug`; UI-этап WOD 2.0 компилируется без ошибок | OK

[INFO] 16.06.2026 | WOD_TASK_UI_STAGE_COMPLETE | Конструктор WOD переведён на задания
и новый WOD отображается атлету на главном экране; следующий шаг - управление справочниками
упражнений, заданий и типов нагрузки | COMPLETE

[INFO] 16.06.2026 | WOD_REFERENCE_QUICK_CREATE_STAGE_START | Начат этап быстрого пополнения
справочников упражнений, типов нагрузки и шаблонов заданий для тренера и администратора | IN_PROGRESS

[INFO] 16.06.2026 | WOD_REFERENCE_API | `AdminApi`, `WodApi`, `AdminRepository` и
`WodRepository` расширены CRUD/create-методами для `exercises`, `load_types` и `training_tasks` | OK

[INFO] 16.06.2026 | WOD_EDITOR_REFERENCE_ACTIONS | В WOD-редактор добавлены действия:
создать упражнение, создать тип нагрузки и сохранить текущее заполненное задание как шаблон | OK

[INFO] 16.06.2026 | WOD_REFERENCE_TRAINER_ADMIN_ACCESS | Тренер и администратор используют
один WOD-редактор, поэтому оба могут пополнять справочники в момент сборки WOD | OK

[INFO] 16.06.2026 | WOD_REFERENCE_BUILD_CHECK | Выполнены `testDebugUnitTest` и
`assembleDebug`; быстрые действия справочников компилируются без ошибок | OK

[INFO] 16.06.2026 | WOD_REFERENCE_QUICK_CREATE_STAGE_COMPLETE | Быстрое пополнение
справочников упражнений, типов нагрузки и шаблонов заданий доступно из WOD-редактора;
полноценные списки редактирования/удаления справочников вынесены в следующий этап | COMPLETE

[INFO] 16.06.2026 | WOD_REFERENCE_ADMIN_CRUD_STAGE_START | Начат этап полноценного
административного управления справочниками упражнений, типов нагрузки и шаблонов заданий | IN_PROGRESS

[INFO] 16.06.2026 | ADMIN_REFERENCE_SECTIONS | В экран администратора `Контент`
добавлены отдельные секции `Упражнения`, `Типы нагрузки` и `Шаблоны заданий` с RecyclerView-списками | OK

[INFO] 16.06.2026 | ADMIN_REFERENCE_CRUD | `AdminContentViewModel` и `ContentFragment`
поддерживают создание, редактирование и удаление упражнений, типов нагрузки и шаблонов заданий | OK

[INFO] 16.06.2026 | ADMIN_REFERENCE_RELATIONS | Диалог шаблона задания использует текущие
справочники упражнений и типов нагрузки, включая optional-варианты | OK

[INFO] 16.06.2026 | ADMIN_REFERENCE_BUILD_CHECK | Выполнены `testDebugUnitTest` и
`assembleDebug`; админский CRUD справочников компилируется без ошибок | OK

[INFO] 16.06.2026 | WOD_REFERENCE_ADMIN_CRUD_STAGE_COMPLETE | Администратор получил
полноценные списки создания, редактирования и удаления упражнений, типов нагрузки и шаблонов заданий;
тренер продолжает использовать быстрые действия справочников из WOD-редактора | COMPLETE

[INFO] 16.06.2026 | ATHLETE_PROGRESS_STAGE_START | Начат этап профиля атлета: личные PR,
история PR и история выполненных WOD на основе новой модели прогресса | IN_PROGRESS

[INFO] 16.06.2026 | ATHLETE_PR_PROFILE_UI | `fragment_profile.xml` расширен карточками
`МОИ PR`, `ИСТОРИЯ PR` и `РЕЗУЛЬТАТЫ WOD`; счетчик PR теперь показывает количество лучших личных рекордов | OK

[INFO] 16.06.2026 | ATHLETE_PR_SUBMIT | `ProfileFragment` и новый `ProfileViewModel` подключены
к `personal_records`: атлет может добавить PR по упражнению, результату, единице, дате и комментарию | OK

[INFO] 16.06.2026 | ATHLETE_WOD_RESULTS_HISTORY | `ResultRepository` и `WodApi` расширены select-запросом
`wods(id,name,scheduled_date,format)`, чтобы профиль показывал историю результатов WOD с названием тренировки | OK

[INFO] 16.06.2026 | ATHLETE_PROGRESS_BUILD_CHECK | Выполнены `lintDebug`, `testDebugUnitTest`
и `assembleDebug`; экран профиля атлета и новые API-контракты компилируются без ошибок | OK

[INFO] 16.06.2026 | ATHLETE_PROGRESS_STAGE_COMPLETE | Профиль атлета получил базовый рабочий контур
для личных рекордов и истории WOD-результатов; следующий шаг - тренерский просмотр PR атлетов и/или отдельные подробные экраны прогресса | COMPLETE

[INFO] 16.06.2026 | TRAINER_ATHLETE_PROGRESS_STAGE_START | Начат этап просмотра прогресса атлетов
тренером из существующего раздела `Атлеты` | IN_PROGRESS

[INFO] 16.06.2026 | TRAINER_ATHLETE_PROGRESS_REPOSITORY | `WodRepository` и `ResultRepository` получили
методы загрузки PR, лучших PR и WOD-результатов по `userId`, чтобы тренер/админ могли смотреть данные выбранного атлета | OK

[INFO] 16.06.2026 | TRAINER_ATHLETE_PROGRESS_UI | `ClientsFragment` и `ClientsAdapter` расширены:
по нажатию на атлета открывается диалог с лучшими PR, историей PR и результатами WOD | OK

[INFO] 16.06.2026 | TRAINER_ATHLETE_PROGRESS_BUILD_CHECK | Выполнены `lintDebug`, `testDebugUnitTest`
и `assembleDebug`; просмотр прогресса атлетов тренером компилируется без ошибок | OK

[INFO] 16.06.2026 | TRAINER_ATHLETE_PROGRESS_STAGE_COMPLETE | Тренер получил базовый просмотр прогресса
активных атлетов из раздела `Атлеты`; следующий шаг - отдельные детальные экраны/фильтры или админская аналитика прогресса | COMPLETE

[INFO] 16.06.2026 | ADMIN_PROGRESS_STAGE_START | Начат этап админского просмотра прогресса
пользователей и унификации UI-компонентов прогресса | IN_PROGRESS

[INFO] 16.06.2026 | PROGRESS_SHARED_COMPONENTS | Добавлены `ProgressFormatter` и `AthleteProgressDialog`;
профиль атлета и тренерский раздел `Атлеты` используют общий формат вывода PR/WOD-результатов | OK

[INFO] 16.06.2026 | ADMIN_USER_PROGRESS_VIEW | В карточку пользователя администратора добавлена кнопка
`ПРОГРЕСС`; `AdminUsersViewModel` загружает лучшие PR, историю PR и WOD-результаты выбранного пользователя | OK

[INFO] 16.06.2026 | ADMIN_PROGRESS_BUILD_CHECK | Выполнены `lintDebug`, `testDebugUnitTest`
и `assembleDebug`; админский просмотр прогресса и общий компонент прогресса компилируются без ошибок | OK

[INFO] 16.06.2026 | ADMIN_PROGRESS_STAGE_COMPLETE | Администратор может смотреть прогресс любого пользователя
из раздела `Пользователи`; следующий шаг - фильтры/полноэкранная детализация прогресса или доработка прав доступа на уровне Supabase | COMPLETE

[INFO] 16.06.2026 | WOD_RESULT_INPUT_STAGE_START | Начат этап улучшения ввода результата WOD
для атлета без изменения серверной схемы | IN_PROGRESS

[INFO] 16.06.2026 | WOD_RESULT_INPUT_PARSER | Добавлен `WodResultInputParser`: для `for_time`
значения вида `12:45` и `1:02:03` автоматически переводятся в секунды для рейтинга; для AMRAP/reps берётся числовой счет | OK

[INFO] 16.06.2026 | WOD_RESULT_INPUT_UI | Диалог результата WOD в `HomeFragment` получил
подсказки по формату WOD и понятную ошибку ввода; после сохранения рейтинг WOD обновляется автоматически | OK

[INFO] 16.06.2026 | WOD_RESULT_INPUT_TESTS | Добавлены unit-тесты `WodResultInputParserTest`
для времени, числовых результатов, десятичных значений и невалидного ввода | OK

[INFO] 16.06.2026 | WOD_RESULT_INPUT_BUILD_CHECK | Выполнены `lintDebug`, `testDebugUnitTest`
и `assembleDebug`; улучшенный ввод результата WOD компилируется и проходит тесты | OK

[INFO] 16.06.2026 | WOD_RESULT_INPUT_STAGE_COMPLETE | Атлет может вводить результат WOD более естественно:
время `mm:ss` для for_time и числовые результаты для остальных форматов; следующий шаг - полноэкранная детализация истории результатов или расширение типов результата | COMPLETE

[INFO] 16.06.2026 | PROGRESS_SECURITY_STAGE_START | Начат этап укрепления RLS/SQL-прав
для просмотра PR и WOD-результатов атлетом, тренером и администратором | IN_PROGRESS

[INFO] 16.06.2026 | PROGRESS_SECURITY_MIGRATION | Добавлена миграция
`20260616_stage9_progress_security.sql`: повторно фиксируются политики `personal_records` и `results`,
удаляется прямое UPDATE-право атлета на `results`, а `personal_record_bests` пересоздаётся через `security_invoker` | OK

[INFO] 16.06.2026 | PROGRESS_BESTS_INACTIVE_EXERCISES | `personal_record_bests` переведён на `LEFT JOIN exercises`,
чтобы лучший PR не исчезал из истории, если упражнение стало неактивным или скрытым политикой `exercises` | OK

[INFO] 16.06.2026 | PROGRESS_SECURITY_DOCS | `database/README.md` дополнен описанием ручной проверки
для атлета, тренера, администратора и запрета PATCH старых WOD-результатов | OK

[INFO] 16.06.2026 | PROGRESS_SECURITY_SQL_CHECK | Выполнен `database/validate_sql.py`:
361 PostgreSQL statements и 14 PL/pgSQL functions проходят синтаксическую проверку | OK

[INFO] 16.06.2026 | PROGRESS_SECURITY_STAGE_COMPLETE | RLS/SQL-контур прогресса закреплён:
атлет видит только своё, тренер - управляемых атлетов, администратор - всё; следующая задача после применения миграции в Supabase - ручная проверка тремя ролями | COMPLETE

[FIX] 16.06.2026 | ATHLETE_PR_TIMEOUT_MITIGATION | После сообщения о таймауте при добавлении PR
профиль атлета перестал зависеть от отдельного запроса к `personal_record_bests`: лучшие PR считаются локально из истории `personal_records`,
а сетевые таймауты PR-запросов получают понятное сообщение. Выполнены `lintDebug`, `testDebugUnitTest`, `assembleDebug` | OK

[FIX] 16.06.2026 | ATHLETE_PR_RPC_404_MITIGATION | После ошибки `HTTP 404` при сохранении PR
...
[INFO] 09.07.2026 | CLIENT_SCHEDULE_CALENDAR_REWORK | Экран расписания атлета переработан:
добавлена верхняя календарная неделя с текущей датой по умолчанию, переключение недель и фильтрация тренировок
по выбранному дню. Список занятий переведён на компактные строки в стиле референса и ARMY UI; тренировки,
на которые клиент уже записан, выделяются красным фоном/акцентом и бейджем `ЗАПИСЬ АКТИВНА`.
Выполнены `lintDebug`, `testDebugUnitTest`, `assembleDebug` | OK

[INFO] 09.07.2026 | SCHEDULE_ALL_ROLES_ACCESS | Проверен доступ к расписанию по ролям:
RLS разрешает всем авторизованным пользователям читать `scheduled` занятия, а Android-навигация теперь
показывает общий экран `Расписание` в нижней панели атлета, тренера и администратора. Для тренера и администратора
экран работает в режиме просмотра без кнопки записи; для атлета сохранены запись и подсветка активных записей.
Выполнены `lintDebug`, `testDebugUnitTest`, `assembleDebug` | OK

[INFO] 09.07.2026 | NEWS_MODULE_STAGE | Добавлен модуль новостей:
миграция `20260709_stage10_news.sql` создаёт `news_posts`, RLS и bucket `news-images`; все роли видят опубликованные
новости, администратор управляет черновиками/публикацией/архивом и выбирает картинку прямо в форме новости.
...
[FIX] 09.07.2026 | BOTTOM_NAV_SPACING_TOP_RESTORE | Removed global `fitsSystemWindows` from main activity roots because it moved
screen headers down; tightened bottom navigation spacing with `itemIconPadding=0dp`, `activeIndicatorLabelPadding=0dp`,
smaller item top/bottom padding and a label text appearance with `includeFontPadding=false` | OK
